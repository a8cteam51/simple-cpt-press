...
class CPT_Press {
	...
	public static function init(): void {
		...
		CPT_Press_Table::init();

		// Register post-type for press items.
		add_action( 'init', array( self::class, 'register_press_items_post_type' ) );

		// Register taxonomies for the post-type.
		add_action( 'init', array( self::class, 'register_press_type_taxonomy' ) );

		// Register post meta for press.
		add_action( 'init', array( self::class, 'register_press_meta' ) );

		// Register the press fields block.
		add_action( 'init', array( self::class, 'register_press_fields_block' ) );

		// Register block patterns.
		add_action( 'init', array( self::class, 'register_block_patterns' ) );

		// Register cpt single template.
		add_action( 'init', array( self::class, 'register_press_item_templates' ) );

		// Register editor scripts
		add_action( 'enqueue_block_editor_assets', array( self::class, 'enqueue_block_editor_assets' ) );

		// Do not allow a single press view.
		add_action( 'template_redirect', array( self::class, 'redirect_single_to_article' ) );

		// Filter the permalink to conditionally use post meta as permalink.
		add_filter( 'post_type_link', array( self::class, 'modify_press_item_permalink' ), 10, 2 );
	}
	...
	public static function enqueue_block_editor_assets(): void {
		...
		if ( $screen && self::POST_TYPE === $screen->post_type ) {
			$assets = wpcomsp_cpt_press_get_asset_meta( WPCOMSP_CPT_PRESS_PATH . 'assets/js/build/index.js' );
			wp_enqueue_script(
	 ...
	 */
	public static function modify_press_item_permalink( string $post_link, WP_Post $post ): string {
	 ...
	 * @since 1.0.0
	 */
	public static function redirect_single_to_article(): void {
	...
	public static function register_block_patterns(): void {
		...
		foreach ( glob( WPCOMSP_CPT_PRESS_PATH . '/patterns/*.html' ) as $pattern_path ) {
			...
			$pattern_title = ucwords( str_replace( '-', ' ', $pattern_info['filename'] ) );
	 ...
	 * @return void
	 */
	public static function register_press_fields_block(): void {
		register_block_type(
			WPCOMSP_CPT_PRESS_PATH . '/blocks/build',
			array( 'render_callback' => array( self::class, 'render_press_fields_block' ) )
		);
	}
	 ...
	 */
	public static function render_press_fields_block( array $attributes, string $content ): string {
