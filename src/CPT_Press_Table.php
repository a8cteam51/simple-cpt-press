...
class CPT_Press_Table {
	...
	public static function init(): void {
		...
		add_filter( 'manage_press_posts_columns', array( self::class, 'add_press_meta_columns' ) );

		// Populate the custom columns with the meta-data.
		add_action( 'manage_press_posts_custom_column', array( self::class, 'populate_press_meta_columns' ), 10, 2 );

		// Make the custom columns sortable.
		add_filter( 'manage_edit-press_sortable_columns', array( self::class, 'make_press_meta_columns_sortable' ) );

		// Modify the query to sort by the custom meta-fields.
		add_action( 'pre_get_posts', array( self::class, 'sort_press_meta_columns' ) );
	}
	 ...
	 */
	public static function add_press_meta_columns( array $columns ): array {
	 ...
	 * @return void
	 */
	public static function populate_press_meta_columns( string $column, int $post_id ): void {
		$colum_value = '';
		switch ( $column ) {
			case 'press_outlet':
	 ...
	 */
	public static function make_press_meta_columns_sortable( array $columns ): array {
	 ...
	 * @return void
	 */
	public static function sort_press_meta_columns( WP_Query $query ): void {
